...
const EditBookSale = () => {
    ...
    const fetchBookSaleInfo = async () => {
        try {
            const response = await axios.get(`http://localhost:3500/api/books/sales/info/${id}`);

            formik.setValues({
                received: response.data.data[0].received,
                due: response.data.data[0].due,
                discount: response.data.data[0].discount,
                quantity: response.data.data[0].quantity,
                buyerInfo: response.data.data[0].buyer_info,
            });
        } catch (error) {
            setShowToast(true);
            setToastStatus("Error");
            setToastMsg("Error fetching book sale info!");
        }
    }

    useEffect(() => {
        fetchBookSaleInfo();
    }, []); // Empty array ensures fetchBookSaleInfo is called only once on mount
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            ...
            try {
                ...
                const response = await axios.patch(`http://localhost:3500/api/books/sales/edit/${id}`, values);
    ...
    return (
        <>
            <div className="main">
                ...
                <div className="right">
                    <Navbar parentPage='Book Sales' parentRoute='/book-sales' childPage='Edit' />
                    <div className="content">

                        <PageHeader pageTitle='Edit Book Sale' actionName='Add Student' actionLink='/students/new' />
                        ...
                        <div className="body">
                            <Form onSubmit={handleSubmit}>
                                ...
                                <Button variant="primary" className="mt-4" type="submit">
                                    {loading ? <Spinner size="sm" animation="border" /> : <PencilFill />} Updaet Book Sale
                                </Button>
                            </Form>

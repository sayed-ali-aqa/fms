...
import { PencilFill, ThreeDotsVertical, LightbulbFill, LightbulbOffFill, FunnelFill, Search, EyeFill } from 'react-bootstrap-icons';
...
const Expenses = () => {
  ...
  const fields = [
    { label: 'Expense Details', key: 'info' },
  ...
  const fetchExpenses = async (pageParam) => {
    try {
      ...
      setToastMsg("Failed to fetch expense list!");
    }
  }

  const fetchExpenseInfo = async (id) => {
    try {
      const response = await axios.get(`http://localhost:3500/api/expenses/info/${id}`);
      setExpenseInfoData(response.data.data[0]);

    } catch (error) {
      setShowToast(true);
      setToastStatus("Error");
      setToastMsg("Failed to fetch expense info!");
    }
  }
  ...
  const getManagers = async () => {
    try {
      const response = await fetch(
        "http://localhost:3500/api/managers/all"
      );
      ...
      setShowToast(true);
      setToastStatus("Error");
      setToastMsg("Failed to fetch manager records!");
    }
  }
  ...
  const activateExpense = async (id) => {
    try {
      const response = await axios.patch(`http://localhost:3500/api/expenses/activate/${id}`);
  ...
  const deactivateExpense = async (id) => {
    try {
      const response = await axios.patch(`http://localhost:3500/api/expenses/deactivate/${id}`);
    ...
    try {
      switch (action) {
        ...
        case "notAllowed":
          setShowToast(true);
          setToastStatus("Error");
          setToastMsg("A deactive expense record cannot be edited!");
          break;

        case "activate":
          const activationConfirmed = confirm('Are you sure to activate the expense record?');
        ...
        case "deactivate":
          const deactivationConfirmed = confirm('Are you sure to deactivate the expense record?');
          if (deactivationConfirmed) {
            deactivateExpense(id);
          }
  ...
  return (
    <>
      <div className="main">
        ...
        <div className="right">
          <Navbar parentPage='Expenses' parentRoute='/expenses' childPage={undefined} />
          <div className="content">

            <PageHeader pageTitle='Expense Lists' actionName='Add Expense' actionLink='/expenses/new' />
            ...
            <div className="body pagination-parent">
              ...
              <Table responsive striped bordered hover>
                ...
                <tbody>
                  {data.length === 0 ? (
                    <tr>
                      <td colSpan="7" className='text-center'>No expense available.</td>
                    </tr>
                  ) : (
                    data.map((row, index) => (
                      <tr key={index}>
                        ...
                        <td>{row.info}</td>
                        <td>{row.amount}</td>
                        <td>{(row.is_active === 1) ? (<span className='text-primary'>Active</span>) : (<span className='text-danger'>Deactive</span>)}</td>
                        ...
                        <td>
                          <Dropdown style={{ position: 'absolute', zIndex: '99' }}>
                            ...
                            <Dropdown.Menu>
                              <Dropdown.Item onClick={() => dropdownAction("view", row.expense_id)}>
                                <EyeFill style={{ marginRight: '4px' }} /> View
                              </Dropdown.Item>

                              {(row.is_active === 1) ?
                                (
                                  <Dropdown.Item onClick={() => dropdownAction("edit", row.expense_id)}>
                                    <PencilFill style={{ marginRight: '4px' }} /> Edit
                                  </Dropdown.Item>
                                ) :
                                (
                                  <Dropdown.Item onClick={() => dropdownAction("notAllowed", row.expense_id)}>
                                    <PencilFill style={{ marginRight: '4px' }} /> Edit
                                  </Dropdown.Item>
                                )
                              }
                              ...
                              {(row.is_active === 1) ? (
                                <Dropdown.Item onClick={() => dropdownAction("deactivate", row.expense_id)}>
                                  <LightbulbOffFill className='text-danger' style={{ marginRight: '4px' }} /> <span className='text-danger'>Deactivate</span>
                                </Dropdown.Item>
                              ) : (
                                <Dropdown.Item onClick={() => dropdownAction("activate", row.expense_id)}>
              ...
              {/* View info modal */}
              <ViewModal
                header="Expense Information"
                keys={fields}
                content={expenseInfoData}
                show={modalShow}
              ...
              <Offcanvas placement={'start'} show={showFilterCanvas} onHide={handleCloseFilterCanvas}>
                <Offcanvas.Header closeButton>
                  <Offcanvas.Title>Filter Expense Data</Offcanvas.Title>
                </Offcanvas.Header>
                <Offcanvas.Body>
                  {/* filter starts */}
                  <div className="filter-container">
                    <form ref={formRef}>
                      <div className="col-md-12 mb-4">
                        <Form.Select aria-label="Select Expense Status" value={selectedData.isActive ? selectedData.isActive : searchParams.get('isActive')} name='status' onChange={(event) => setSelectedData({ ...selectedData, isActive: event.target.value })}>
              ...
              <Offcanvas placement={'start'} show={showSearchCanvas} onHide={handleCloseSearchCanvas}>
                <Offcanvas.Header closeButton>
                  <Offcanvas.Title>Search Expense Data</Offcanvas.Title>
                </Offcanvas.Header>
                <Offcanvas.Body>
                  {/* filter starts */}
                  <div className="filter-container">
                    <form ref={formRef}>
                      <div className="col-md-12 mb-4">
                        ...
                        {((searchedData.selectedField !== "" && searchedData.selectedField === "manager") || (searchParams.get('selectedField') === "manager" && searchParams.get('searchedValue') !== "")) ?
                          (
                            <>
                              <label htmlFor="searchedValue" className='mt-3'>Select manager</label>
                              <Form.Select className='mt-1' aria-label="Select manager" value={searchedData.searchedValue ? searchedData.searchedValue : searchParams.get('searchedValue')} onChange={(event) => setSearchedData({ ...searchedData, searchedValue: event.target.value })}>
